...
기본 코드 예제는 예시 도메인을 사용하지만, 실습의 목적은 **사용자가 추가로 입력한 업무 목표를 이루는 에이전트**를 만드는 방법을 익히는 데 있습니다.
...
`----` 사이에는 사용자가 직접 넣는 목표와 배경 자료를 적습니다.
...
아래 사용자가 넣은 내용을 먼저 읽고, 이 실습이 어떤 흐름으로 진행되는지 6단계로 요약해라.
내가 만들 업무 목표를 1문장으로 정리하고, 기존 자료와 기존 프롬프트에서 재사용할 만한 부분도 함께 적어라.
아직 코드는 수정하지 마라.
```
...
이 실습은 LLM을 단순한 글쓰기 도구로 보지 않습니다. LLM은 창의성, 추론, 지식 결합을 맡고, 시스템은 상태, 검증, 기록, 실행을 맡습니다. Agent는 그 사이에서 반복 업무를 구조화하고 정보를 표준화하는 역할을 합니다.

그래서 차세대 개발은 "코드를 더 빨리 쓰는 일"보다 "업무와 정보를 더 잘 구조화해서 Agent가 운영하게 만드는 일"에 가깝습니다. 반복 작업은 Skill과 프롬프트로 고정하고, 초안 생성은 Agent가 맡고, 승인과 브리핑은 시스템이 관리합니다.

## 다음 단계 실습

이 흐름을 익힌 뒤에는 아래 순서로 더 확장할 수 있습니다.

1. 요청 입력을 더 세분화한다. 예: 목표, 기존 자료, 제약 조건, 우선순위
2. 반복 업무를 Skill로 분리한다. 예: 요약, 답변 초안, 검증, 브리핑
3. Agent를 역할별로 나눈다. 예: 기획 Agent, 생성 Agent, 검수 Agent
