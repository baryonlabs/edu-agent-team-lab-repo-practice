...
기본 예제는 예시 도메인을 사용하지만, 실제 실습의 목표는 사용자가 추가로 입력한 업무 목표를 수행하는 범용 에이전트를 조립하는 방법을 익히는 것입니다.
...
- 프롬프트: `prompts/03-skill-pattern.md`
- 산출물: `skills/response-pattern.md`
- 알아야 할 것: Skill은 반복 프롬프트가 아니라 재사용 가능한 업무 절차입니다. 현재 파일명은 예제 유산이고, 내용은 사용자 목표에 맞게 바꿔 읽습니다.
- 확인 명령: `sed -n '1,160p' skills/response-pattern.md`

### Step 4. 단일 Agent

- 프롬프트: `prompts/04-agent-build.md`
- 산출물: `backend/db.py`, `backend/schema.sql`, `backend/seed_input.py`, `agents/task_agent.py`
